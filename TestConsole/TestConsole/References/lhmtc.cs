...
namespace TestConsole.References
{
    ...
    public class lhmtc
    {
        ...
        public struct TMDComparePrm
        {
            ...
            short encz;
            short enca;
            short source;          //比较源： 0：规划   1:反馈
            short outputType;      //输出方式：0：脉冲  1：电平
            short startLevel;      //起始电平
            short time;            //比较输出脉冲上升沿宽度   单位100us
            short maxerr;          //比较范围最大误差
            short threshold;       //最优算法阈值
            short pluseCount;      //输出脉冲个数
            short spacetime;       //脉冲下降沿宽度
            short delaytime;       //输出延时时间
        };
        ...
        public struct TMDCompareDataEX
        {
            ...
            long pz;
            long pa;
            short time;           //上升沿宽度  单位100us
            short spacetime;      //下降沿宽度
            short delaytime;      //延时时间
            short pluseCount;     //脉冲个数
        };
        #endregion
        ...
        [DllImport("lhmtc.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern short LH_MDCompareBindOn(short chn, short time,long lPluseCount,short spacetime,short inputIo,short senselevel,short delayTime,short cardNum);
        [DllImport("lhmtc.dll", CallingConvention = CallingConvention.Cdecl)] 
